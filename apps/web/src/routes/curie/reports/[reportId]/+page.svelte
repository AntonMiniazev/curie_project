<script lang="ts">
	...
	const streamlitUrl = $derived(report ? `${streamlitBaseUrl}${report.streamlit_path}` : null);
			...
			<section class="curie-card">
				...
				<div class="grid min-h-[560px] place-items-center p-6">
					<div class="max-w-xl text-center">
						<h3 class="text-xl font-semibold text-[var(--curie-text)]">Embed placeholder</h3>
						<p class="mt-3 leading-7 text-[var(--curie-text-muted)]">
							This area is reserved for a Streamlit iframe once the report apps are deployed. The
							frontend already knows the target path from PostgreSQL report metadata.
						</p>
					</div>
				</div>
			</section>
